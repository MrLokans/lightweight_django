...
class SprintSerializer(serializers.ModelSerializer):
    ...
    # every field may have corresponding validate_<field_name> method
    def validat_end(self, attrs, source):
        end_date = attrs[source]
        is_new = not self.object
        is_changed = self.object and self.object.end != end_date

        if (is_new or is_changed) and (end_date < date.today()):
...
class UserSerializer(serializers.ModelSerializer):
    ...
    def get_links(self, obj):
        ...
        return {
            ...
            'tasks': reverse('{}?assigned={}'.format(reverse('task-list', request=request), username))
        }
